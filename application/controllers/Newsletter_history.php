...
class Newsletter_history extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $newsletter_activity = $this->Subscriber_history_model->newsletter_get_all();

        $data = array(
            'newsletter_activity_data' => $newsletter_activity
        );
		//echo "<pre>"; 
		//print_r( $data);exit;
        // $this->load->view('users_activity/users_activity_list', $data);
		$this->breadcrumbs->push('Logs', '/admin_logs');
		$this->breadcrumbs->push('Newsletter Activity List', '/Newsletter_history/index');
         $this->commonmodel->adminloadLayout($data,'subscriber_history/Newsletter_history');
    }
    ...
    public function delete($id)
    {
        ...
        $row = $this->Subscriber_history_model->newsletter_get_by_id($id);
        if ($row) {
            $this->Subscriber_history_model->delete_newsletter_subscriber($id);
            $this->session->set_flashdata('message', '<div class="alert alert-success">Record Deleted Success</div>');
            redirect(site_url('Newsletter_history'));
        } else {
    ...
    public function excel()
    {
        ...
        $namaFile = "Newsletter_Subscribers.xls";
        $judul = "Newsletter Subscriber Activity";
        $tablehead = 0;
        $tablebody = 1;
        $nourut = 1;
        //penulisan header
	...
	foreach ($this->Subscriber_history_model->newsletter_get_all() as $data) {
    ...
    public function word()
    {
        ...
        header("Content-Disposition: attachment;Filename=Newsletter_Subscribers.doc");

        $data = array(
            'newsletter_activity_data' => $this->Subscriber_history_model->newsletter_get_all(),
            'start' => 0
        );

        $this->load->view('subscriber_history/newsletter_doc',$data);
    }
